 ...
 * the wrapper forces it onto its own line within the parent's `flex flex-wrap` actions row (same
 * row as "Unir en un documento"), instead of squeezing next to the buttons.
 */
export function ProjectChatButton({ projectId, projectName }: { projectId: string; projectName: string }) {
    ...
    <>
      ...
      {open && (
        <div className="w-full">
          <ChatPanel defaultScope="project" projectId={projectId} projectName={projectName} />
        </div>
      )}
    </>
  );
